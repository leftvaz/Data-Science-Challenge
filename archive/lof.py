# -*- coding: utf-8 -*-
"""
Created on Thu Mar 22 11:56:17 2018
...
np_scaled = min_max_scaler.fit_transform(data)
# train one class SVM 
model = LocalOutlierFactor(n_neighbors=5, contamination = outliers_fraction) #nu=0.95 * outliers_fraction  + 0.05
data = pd.DataFrame(np_scaled)
model.fit(data)
# add the data to the main  
...
a = df.loc[df['anomaly26'] == 1, ['timestamp', 'acceleration', 'speed', 'Journey_ID']] #anomaly
